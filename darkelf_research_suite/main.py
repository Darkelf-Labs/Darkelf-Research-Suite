...
# Dedicated Tor data directory
DARKELF_TOR_DIR = Path.home() / ".darkelf_research" / "tor"
DARKELF_TOR_DIR.mkdir(parents=True, exist_ok=True)
...
class TorManager:
    ...
    def start_tor(self):
        ...
        try:
            ...
            console.print(
                f"[green]Tor started successfully on SOCKSPort "
                f"{self.socks_port} and ControlPort "
                f"{self.control_port}[/green]"
            )
    ...
    def stop_tor(self):
        """Stop Tor only if Darkelf started it."""
        ...
        if self.tor_controller:
            try:
                ...
                _log(f"Failed to close Tor controller: {e}", "WARN")

        # Only terminate Tor if Darkelf launched it
        if self.started_by_darkelf and self.tor_process:
            try:
                self.tor_process.terminate()
                console.print("[yellow]Tor process terminated.[/yellow]")
            except Exception as e:
                _log(f"Failed to terminate Tor process: {e}", "WARN")
...
class DarkelfResearchAI:
    ...
    def __init__(self, model="mistral"):
        ...
        if not self.ollama_path:
            ...
            console.print(
                "[yellow]Ollama not installed. AI features disabled.[/yellow]"
            )
            return
...
class DarkelfCLI:
    def __init__(self):
        ...
        # First, try to connect to an existing Tor instance
        try:
            self.tor_manager.tor_controller = Controller.from_port(
                port=self.tor_manager.control_port
            )
            self.tor_manager.tor_controller.authenticate()

            self.tor_enabled = True

            console.print(
                "[green]✓ Connected to existing Tor instance.[/green]"
            )

        except Exception:
            # No existing Tor found; try starting one
            try:
                self.tor_manager.start_tor()
                self.tor_enabled = True

                console.print(
                    "[green]✓ Started a new Tor instance.[/green]"
                )
                ...
                _log(
                    f"Unable to connect to or start Tor: {e}",
                    "WARN",
                )

                console.print(
                    "[yellow]Tor is unavailable. "
                    "Onion features have been disabled. "
                    "All other Darkelf Research Suite features remain available."
                    "[/yellow]"
                )

        # ---------------------------
        # Initialize WHOIS / DNS
        # ---------------------------
